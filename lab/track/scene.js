...
const CAR_MODEL = { urls: ['./models/gt3.glb', './models/2024_porsche_992_gt3_r/scene.gltf', './models/gt3/scene.gltf'], rotateY: 0 };
...
export function createTrackScene(canvas, options = {}) {
  ...
  // --- real track model (replaces the procedural road, ground and props) ---
  if (trackOpt && trackOpt.model) {
    loadTrackModel(trackOpt.model).then(({ root, asphaltMat, triangles }) => {
      scene.add(root);
  ...
  // --- cars: the one ahead and the player (visible in chase view) ---
  const PAINT_AHEAD = 0xd8342a, PAINT_PLAYER = 0x1d4fc4;
  let carAhead = buildProceduralGT3(PAINT_AHEAD);
  let playerCar = buildProceduralGT3(PAINT_PLAYER);
  playerCar.visible = false;
  scene.add(carAhead, playerCar);
  const swapCar = (which, next) => {
    const old = which === 'ahead' ? carAhead : playerCar;
    next.visible = old.visible; scene.remove(old); scene.add(next);
    if (which === 'ahead') carAhead = next; else playerCar = next;
    setTime(state.hour);
  };
  // try the real model; keep the procedural one if it is not there
  (async () => {
    let url = null;
    for (const u of CAR_MODEL.urls) { try { if ((await fetch(u, { method: 'HEAD' })).ok) { url = u; break; } } catch (e) { /* keep looking */ } }
    if (!url) { console.info('[track] no car model in lab/track/models — using the procedural GT3'); return; }
    try {
      const [a, p] = await Promise.all([
        loadCarModel(url, { rotateY: CAR_MODEL.rotateY, paint: PAINT_AHEAD }),
        loadCarModel(url, { rotateY: CAR_MODEL.rotateY, paint: PAINT_PLAYER }),
      ]);
      swapCar('ahead', a); swapCar('player', p);
      setCameraMode(state.cameraMode);
      renderer.compileAsync(scene, camera).catch(() => {});
      console.info(`[track] car model loaded: ${url} (${a.userData.triangles.toLocaleString()} tris)`);
    } catch (e) { console.warn('[track] car model failed to load, keeping the procedural GT3:', e); }
  })();
  ...
  if (!isMobile) {
    ...
    bloom = new UnrealBloomPass(new THREE.Vector2(1, 1), 0.35, 0.5, 0.86);
  ...
  // ---------- state ----------
  const state = {
    hour: 18.8, rain: 0, progress: 0, cameraMode: 'helmet',
    uSmooth: 0, uPrev: 0, speedMps: 0, steer: 0, t: 0,
  ...
  function setTime(hour) {
    ...
    hemi.intensity = 0.06 + daylight * 0.3;
    hemi.color.setHSL(0.6, 0.4, 0.55 + 0.2 * daylight);
    renderer.toneMappingExposure = 0.3 + daylight * 0.18;
    ...
    if (bloom) bloom.strength = 0.25 + night * 0.55;
    refreshEnvironment();
  }
  ...
  const tmpP = new THREE.Vector3(), tmpT = new THREE.Vector3(), tmpS = new THREE.Vector3(), look = new THREE.Vector3();
  ...
  function frame() {
    ...
    state.uSmooth += (target - state.uSmooth) * (reduced ? 1 : Math.min(1, dt * 4.5));
    ...
    const k = curvatureAt(curve, u);
    state.steer += (THREE.MathUtils.clamp(k * 0.35, -1, 1) - state.steer) * Math.min(1, dt * 6);

    // player car on the spline (its pose defines every camera)
    curve.getPointAt(u, tmpP); curve.getTangentAt(u, tmpT); tmpS.crossVectors(tmpT, UP).normalize();
    const speedN = THREE.MathUtils.clamp(state.speedMps / 70, 0, 1);
    playerCar.position.copy(tmpP).addScaledVector(tmpS, -0.4);
    playerCar.lookAt(playerCar.position.clone().add(tmpT));
    playerCar.rotateZ(-state.steer * 0.02);
    ...
    let lookAhead = 14, height = 1.15;
    const off = state.camOffsets[mode] || state.camOffsets.helmet;
    if (mode === 'chase') { camLocal.set(...off); lookAhead = 22; height = 2.6; }
    else if (mode === 'hood') { camLocal.set(...(real ? off : [0, 0.85, 1.2])); height = off[1]; }
    else { camLocal.set(...(real ? off : [0, 1.15, 0])); height = real ? off[1] : 1.15; }
    camPos.copy(camLocal).applyMatrix4(playerCar.matrixWorld);
    camPos.y += reduced ? 0 : Math.sin(state.t * 9) * 0.006 * (0.3 + speedN);
    camera.position.copy(camPos);
    curve.getPointAt((u + lookAhead / trackLength) % 1, look);
    look.y += height * 0.8 + 0.2 - (off[3] || 0);
    look.addScaledVector(tmpS, mouse.x * 1.6 + state.steer * 0.8);
    look.y -= mouse.y * 0.5;
    camera.lookAt(look);
    camera.rotateZ(-state.steer * 0.035 + (reduced ? 0 : Math.sin(state.t * 7.3) * 0.002 * speedN));
    camera.fov += ((mode === 'chase' ? 58 : mode === 'helmet' ? 72 : 66) + speedN * 8 - camera.fov) * Math.min(1, dt * 3);
    camera.updateProjectionMatrix();

    rig.userData.wheel.rotation.z = state.steer * 1.4;
    if (playerCar.userData.steeringWheel) playerCar.userData.steeringWheel.rotation.z = state.steer * 1.4;
  ...
  return { setTime, setRain, setPaint, setCarAheadPaint, setCameraMode, setProgress, onFrame, trackOutline, trackPoint2D, trackLength, state, get cars() { return { ahead: carAhead, player: playerCar }; }, _debug: { scene, renderer, get composer() { return composer; }, sunLight, camera } };
}
