...
- `projects/index.html` — the showcase grid plus the full project log; `projects/<slug>.html` — one page per project (`terminal.html` is the template). A project page is the blog post; there is no separate blog. Numbering is by build date: 001 Terminal (Nov 2021), 002 Vent (Jul 2023), 003 Spare (Nov 2025), 004 Chain (Jul 2026), 005 Rep Zero (Sep 2026). Personal “why” paragraphs and lessons the author has not written stay as `[BRACKETS]`. Vent lives at `vent-tool.vercel.app` (the repo’s homepage field is stale).
